...
pub async fn run_filter(config: &AppConfig) -> Result<()> {
    ...
    if config.filtered_list_file().exists() {
        ...
        if nc > 0 {
            ok(&format!("{nc} mod(s) already classified — skipping their API calls."));
        } else {
    ...
    info(&format!("Already classified   : {} (skipped)", total_cached.to_string().bold()));
    info(&format!("Querying Thunderstore: {} mods…", total_api.to_string().bold()));
    if total_api > 0 {
    ...
    for entry in mods_need_api.iter() {
        ...
        let category = thunderstore::classify_mod(&client, &ns, &name).await
    ...
    separator_n(44);
    println!("  {}  Client-side only   : {}", "✘".red(),   mod_client.len().to_string().bold());
    println!("  {}  Server-side only   : {}", "✔".green(), mod_server.len().to_string().bold());
    println!("  {}  Both (client+server): {}", "◈".green(), mod_both.len().to_string().bold());
    println!("  {}  Unknown category   : {}", "?".yellow(), mod_unknown.len().to_string().bold());
    separator_n(44);

    let combined = dedup_list(
        mod_server.iter().chain(mod_both.iter()).chain(mod_unknown.iter()),
    );
    ...
    out.push_str(&format!(
        ...
        mod_client.len(), mod_server.len(), mod_both.len(), mod_unknown.len()
    ));
    out.push_str("## Client-side only\n");
    for m in &mod_client { out.push_str(m); out.push('\n'); }
    out.push('\n');
    out.push_str("## Server-side only\n");
    for m in &mod_server { out.push_str(m); out.push('\n'); }
    out.push('\n');
    out.push_str("## Both\n");
    for m in &mod_both { out.push_str(m); out.push('\n'); }
    out.push('\n');
    out.push_str("## Unknown\n");
    for m in &mod_unknown { out.push_str(m); out.push('\n'); }
    out.push('\n');
    out.push_str("## Server-side + Both + Unknown (no duplicates)\n");
    for m in &combined { out.push_str(m); out.push('\n'); }
    ...
    if !mod_client.is_empty() {
        ...
        for m in &mod_client { println!("    {}  {}", "✘".red(), m); }
        println!();
    }
    if !mod_server.is_empty() {
        println!("  \x1b[0;32m\x1b[1mServer-side only:\x1b[0m");
        for m in &mod_server { println!("    {}  {}", "✔".green(), m); }
        println!();
    }
    if !mod_both.is_empty() {
        println!("  \x1b[0;32m\x1b[1mBoth (client + server):\x1b[0m");
        for m in &mod_both { println!("    {}  {}", "◈".green(), m); }
        println!();
    }
    if !mod_unknown.is_empty() {
        println!("  \x1b[1;33m\x1b[1mUnknown:\x1b[0m");
        for m in &mod_unknown { println!("    {}  {}", "?".yellow(), m); }
        println!();
    }

    println!(
        "  \x1b[0;36m\x1b[1mServer-side + Both + Unknown (no duplicates) — {} mods:\x1b[0m", combined.len()
    );
    for m in &combined { println!("    {}  {}", "◈".cyan(), m); }
    println!();
    ...
    if !confirm(&format!(
        ...
        println!(
            "  \x1b[0;36m   then run {} to install the filtered set.\x1b[0m", "odin install-mods".bold()
        );
        separator_n(44);
        return Ok(());
    }
    ...
    if !mod_unknown.is_empty() {
        println!();
        println!(
            "  \x1b[1;33m⚠  {} Unknown mod(s) were not automatically categorized:\x1b[0m", mod_unknown.len()
        );
        separator_n(44);
        for m in &mod_unknown { println!("    {}  {}", "?".yellow(), m); }
        separator_n(44);
        println!("  \x1b[0;36mIncluding them preserves the previous behaviour (safe default).\x1b[0m");
        println!();
        ...
        if confirm(&format!(
            ...
            ok(&format!("{unknown_included} unknown mod(s) added to the selection."));
        } else {
    ...
    list_out.push_str(&format!(
        ...
        mod_server.len(), mod_both.len(), unknown_included, final_list.len()
    ));
    for m in &final_list { list_out.push_str(m); list_out.push('\n'); }

    fs::write(config.mods_list_file(), &list_out).map_err(Error::Io)?;
    ok(&format!("mods_list.txt updated with {} mod(s).", final_list.len().to_string().bold()));
    ...
    println!("  {}  Server-side only     : {}", "✔".green(), mod_server.len().to_string().bold());
    println!("  {}  Both (client+server) : {}", "◈".green(), mod_both.len().to_string().bold());
    println!("  {}  Removed (client-only): {}", "✘".red(),   mod_client.len().to_string().bold());
    println!("  {}  Unknown mods         : {}", "?".yellow(), mod_unknown.len().to_string().bold());
    println!("  {}  Total in mods_list   : {}", "◈".cyan(),   final_list.len().to_string().bold());
    separator_n(44);
    println!();
    println!(
        "  \x1b[1;33m▶  Next step:\x1b[0m Run {} to install the updated mod list.", "odin install-mods".bold()
    );
    separator_n(44);
    Ok(())
}
...
pub async fn run_download(config: &AppConfig) -> Result<()> {
    ...
    separator_n(44);
    ok(&format!("Download phase complete → {}", config.mods_cache_dir().display()));
    println!("  \x1b[0;32m↓  Freshly downloaded  : {}", downloaded.len().to_string().bold());
    println!("  \x1b[0;36m◈  Already in cache    : {}\x1b[0m", cached.len().to_string().bold());
    if !changes.is_empty() {
        println!("  \x1b[1;33m↑  Versions migrated   : {}\x1b[0m", changes.len().to_string().bold());
    }
    if !skipped.is_empty() {
        println!("  \x1b[1;33m⊘  Marked (*) ignored  : {}\x1b[0m", skipped.len().to_string().bold());
    }
    if !failed.is_empty() {
        println!("  \x1b[0;31m✘  Failed              : {}\x1b[0m", failed.len().to_string().bold());
    }
    separator_n(44);
    println!(
...
pub async fn run_install(config: &AppConfig) -> Result<()> {
    ...
    if plugins_dir.is_dir() && !dir_is_empty(&plugins_dir) {
        println!();
        warn(&format!("Plugins directory is not empty: {}", plugins_dir.display()));
    ...
    ok(&format!("Plugins directory ready: {}", plugins_dir.display()));
    ...
    println!("  \x1b[0;32m✔  Total installed     : {}\x1b[0m", to_extract.len().to_string().bold());
    println!("  \x1b[0;36m◈  From cache          : {}\x1b[0m", cached.len().to_string().bold());
    println!("  \x1b[0;32m↓  Freshly downloaded  : {}\x1b[0m", downloaded.len().to_string().bold());
    if !changes.is_empty() {
        println!("  \x1b[1;33m↑  Versions migrated   : {}\x1b[0m", changes.len().to_string().bold());
    }
    if !skipped.is_empty() {
...
pub async fn run_clear(config: &AppConfig) -> Result<()> {
    ...
    if state == "running" {
        ...
        info(&format!("Server is not running (state: {state}). No action needed."));
    }
    ...
    if worlds_local.is_dir() && !dir_is_empty(&worlds_local) {
        ...
        info(&format!("Backing up {} → {}…", worlds_local.display(), backup_path.display()));

        let status = sudo_run_bool(&["7z", "a", &backup_path.to_string_lossy(), &format!("{}/.", worlds_local.display()), "-tzip", "-y"])?;
        if !status {
            return Err(Error::other("Backup failed! Aborting to protect world data."));
        }
    ...
    let bepinex_dir = config.plugins_dir().parent().map(|p| p.to_path_buf())
        .unwrap_or_else(|| config.script_dir.join("config/bepinex"));

    let items: Vec<(&str, PathBuf, bool)> = vec![
        ("Mods list file        (mods_list.txt)", config.mods_list_file(), false),
        ("Mods cache dir        (mods_cache/*)", config.mods_cache_dir(), true),
        ("Server data dir       (data/*)", data_dir.clone(), true),
        ("BepInEx config dir    (config/bepinex/*)", bepinex_dir, true),
    ];
    ...
    for (label, path, is_dir) in &items {
        ...
        if confirm(&format!("{}Delete this {}? (y/N){}", "\x1b[0;31m", if *is_dir { "dir" } else { "file" }, "\x1b[0m")) {
            if *is_dir {
                sudo_rm_rf(path)?;
            } else {
    ...
    println!();
    separator_n(44);
    println!("  \x1b[0;32m✔  Deleted : {}\x1b[0m", deleted.to_string().bold());
    println!("  \x1b[1;33m⊘  Skipped : {}\x1b[0m", skipped.to_string().bold());
    if absent > 0 {
        println!("  \x1b[0;36m◈  Absent  : {}\x1b[0m", absent.to_string().bold());
    }
    separator_n(44);
    ...
    if let Some(ref bp) = latest_backup {
        let bp_name = bp.file_name().unwrap_or_default().to_string_lossy().to_string();
        ...
        if confirm(&format!("{}Restore this backup to ./config/worlds_local? (n/Y){}", "\x1b[0;32m", "\x1b[0m")) {
            if worlds_local.exists() {
                sudo_rm_rf(&worlds_local)?;
                info("Removed existing worlds_local.");
            }
            sudo_mkdir_p(&worlds_local)?;
            info(&format!("Extracting {bp_name} → {}…", worlds_local.display()));
            let ok_flag = sudo_run_bool(&["7z", "x", &bp.to_string_lossy(), &format!("-o{}", worlds_local.display()), "-y"])?;
            if ok_flag {
                ok(&format!("World restored successfully from {}.", bp_name.bold()));
                ok("Your latest Valheim progress is preserved.");
            } else {
                err(&format!("Extraction failed. Please restore manually from: {}", bp.display()));
            }
        } else {
            warn(&format!("Restore skipped. Backup remains available in {}.", backups_dir.display()));
        }
    } else {
        warn(&format!("No backup found in {}. Skipping restore.", backups_dir.display()));
    }

    println!();
    separator_n(44);
    if deleted > 0 { ok("Cleanup complete."); } else { info("Nothing was deleted."); }
    println!();
    println!(
        "  \x1b[1;33m▶  Next step:\x1b[0m Run {} to launch a vanilla Valheim server (no mods).", "odin start".bold()
    );
    println!(
        "  \x1b[0;36m   To reinstall mods afterwards, run {}.\x1b[0m", "odin install-mods".bold()
    );
    separator_n(44);
    Ok(())
}
...
async fn download_mods(
    ...
    section(&format!("Downloading  ({total} entries)  [API-resolved, always latest]"));
    ...
    for raw_entry in all_mods.iter() {
        ...
        if dash_count < 2 {
            pb.println(format!("  ⊘ Invalid entry (not Author-Mod-Version format): '{entry}'"));
        ...
        if let Some(cached_zip) = find_cached_zip(&cache_dir, &base) {
            ...
            if cached_name == resolved_name {
                if zip_valid(&cached_zip) {
                    ...
                    pb.println(format!("  ⊘ Corrupt cached zip, re-downloading: {}", cached_zip.display()));
        ...
        if wget_download(&resolved_url, &dest) {
            if zip_valid(&dest) {
                ...
                pb.println(format!("  ✘ Downloaded archive is invalid: {resolved_name}"));
    ...
    separator_n(44);
    println!("  \x1b[0;32m✔\x1b[0m  Downloaded: {}", downloaded.len().to_string().bold());
    if !cached.is_empty() {
        println!("  \x1b[0;36m◈\x1b[0m  Cached (skipped): {}", cached.len().to_string().bold());
    }
    if !skipped.is_empty() {
        println!("  \x1b[1;33m⊘\x1b[0m  Marked (*) / invalid: {}", skipped.len().to_string().bold());
    }
    if !failed.is_empty() {
        println!("  \x1b[0;31m✘\x1b[0m  Failed: {}", failed.len().to_string().bold());
    }
    separator_n(44);

    if !failed.is_empty() {
        for f in &failed { err(&format!("  ↳ {f}")); }
        println!();
...
fn find_cached_zip(cache_dir: &Path, base: &str) -> Option<PathBuf> {
    ...
    fs::read_dir(cache_dir).ok()?.flatten()
        .filter(|e| {
...
fn detect_version_changes(mods: &[String], cache_dir: PathBuf) -> (Vec<String>, Vec<PathBuf>) {
    ...
    for raw in mods {
        ...
        if entry.ends_with("**") { entry = entry.trim_end_matches("**").to_string(); }
        if entry.ends_with('*') { continue; }
        if entry.chars().filter(|&c| c == '-').count() < 2 { continue; }

        let base = strip_version(&entry);
        if let Some(old_zip) = find_cached_zip(&cache_dir, &base) {
            let old_name = old_zip.file_stem().unwrap_or_default().to_string_lossy().to_string();
            if old_name != entry {
                changes.push(format!("{old_name}  →  {entry} (will resolve latest via API)"));
                purge.push(old_zip);
            }
        }
    }
...
fn confirm_migration(changes: &[String], purge: &[PathBuf]) -> bool {
    ...
    separator_n(44);
    for c in changes { println!("  \x1b[0;36m◈\x1b[0m  {}", c); }
    separator_n(44);
...
fn purge_old_zips(paths: &[PathBuf]) {
    for p in paths {
        ...
        info(&format!("Cache purged: {}", p.file_name().unwrap_or_default().to_string_lossy()));
    }
}
...
fn find_latest_backup(backups_dir: &Path) -> Option<PathBuf> {
    let mut files: Vec<_> = fs::read_dir(backups_dir).ok()?.flatten()
        .filter(|e| {
            let n = e.file_name().to_string_lossy().to_string();
...
fn sudo_7z_extract(zip: &Path, dest: &Path) -> Result<()> {
    ...
    match status {
        ...
        Ok(s) => Err(Error::other(format!("7z extract failed (exit {:?})", s.code()))),
        Err(e) => Err(Error::other(format!("exec error: {e}"))),
    }
}
...
fn make_progress_bar(total: u64) -> ProgressBar {
    ...
    pb.set_style(
        ProgressStyle::with_template(
            "  [{bar:40.cyan/blue}] {pos:>3}/{len}  {msg}",
        )
        .unwrap()
        .progress_chars("█▓░"),
    );
